...
class PersianCalendarWidget(QTableWidget):
    ...
    def populate(self):
        ...
        else: days_in_month = 30 if jdatetime.date.is_leap(year) else 29
        ...
        for row in range(6):
            for col in range(7):
                ...
                item.setData(Qt.ItemDataRole.UserRole, jdatetime.date(year, month, day))
...
class CalendarPopup(QWidget):
    ...
    def __init__(self, parent=None, year_range_past=0, year_range_future=0, show_time_selector=False):
        super().__init__(parent)
...
class InvoiceDatePicker(DatePicker):
    ...
    def __init__(self, parent=None):
        ...
        self.popup = CalendarPopup(self, year_range_future=1, show_time_selector=True)
